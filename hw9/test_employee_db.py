...
def test_to_get_info():
    ...
    db.create_employee(first_name, last_name, middle_name, company_id, email, phone, birthdate, is_active)
    employee_db = db.get_info()
    employee_api = api.get_info()["id"]
    db.delete(employee_db)

    assert employee_db == employee_api
